...
- MediaPipe assets (WASM + face/hand/object models) are **self-hosted**: `frontend/scripts/sync-mediapipe-assets.mjs` runs on `predev`/`prebuild`, copies the WASM runtime out of `node_modules` and downloads the three SHA256-pinned models into `public/mediapipe/` (cached in `frontend/vendor/`, both gitignored). `lib/mediapipeAssets.ts` tries the local copy first and falls back to the public CDN; if every source fails the engines report `onReady(false)`, the student sees a degraded-monitoring banner and the client POSTs `/api/student/proctor-engine-status` so `StudentExam.proctor_engine_status` shows it in Django admin. Same pattern as the Silero VAD assets (`sync-vad-assets.mjs`).
- MediaPipe **inference runs in a Web Worker** (`lib/proctorWorker.ts` + `lib/proctorWorkerClient.ts`); only inference moves, all decision logic (gaze thresholds, escalation, violations) stays on the main thread, so changing detection rules never touches the worker. Frames cross as transferable `ImageBitmap`. `ProctorWorkerClient.create()` returns `null` — never throws — when the browser lacks Worker/createImageBitmap/OffscreenCanvas, when `VITE_PROCTOR_WORKER=0`, or when init times out; callers then fall back to the original main-thread path, so the worst case equals the pre-worker behavior. Requires `worker: { format: 'es' }` in `vite.config.ts` (the worker dynamically imports `@mediapipe/tasks-vision`, which Vite's default `iife` worker format cannot code-split).
